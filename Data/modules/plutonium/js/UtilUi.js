...
class UtilUi {
	...
	static _init_addDirectoryWrapper (hookName) {
		...
		Hooks.on(hookName, (app, $html) => {
			...
			const $ptBtnsImport = isAddImportButton ? $$`<div class="btn-group flex-v-center mr-1 w-100">
				${$btnImport ? $btnImport.addClass(`dir__btn-header`) : null}
				${$btnQuick ? $btnQuick.addClass(`dir__btn-header`) : null}
			</div>` : `<div class="w-100"></div>`;

			const $wrp = $$`<div class="flex-col dir__wrp-header w-100 no-shrink min-w-100 dir__control-header ${this._init_isRequireExtraSpacingFvttEntityType(hookName) ? "pb-1" : ""}">
	...
	static _init_isImportableFvttEntityType (hookName) {
		return !["renderMacroDirectory", "renderCompendiumDirectory"].includes(hookName);
	}
