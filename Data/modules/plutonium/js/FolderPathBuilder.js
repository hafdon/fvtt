...
class FolderPathBuilder extends BaseComponent {
	...
	render ($parent) {
		...
		this._$wrpRows = $$`<div class="flex-col w-100 h-100 overflow-y-auto relative"></div>`;
	...
	_render_checkAddEmptyMessage () {
		if (!this._childComps.length) this._$wrpRows.append(`<div class="flex-v-center w-100 my-1"><i>(Directory root)</i></div>`);
	}
...
class FolderPathBuilderRowTextOnly extends BaseComponent {
	...
	render ($parent, parent) {
		...
		this._$row = $$`<div class="flex-v-center w-100 my-1 imp-folder__row">
			${$iptName}
...
class FolderPathBuilderRow extends FolderPathBuilderRowTextOnly {
	render ($parent, parent) {
		...
		if (this._fnOnKeydown) $iptName.keydown(evt => this._fnOnKeydown(evt));
		const $wrpFreeText = $$`<div class="flex mr-1 w-100">${$iptName}</div>`;
		...
		const $wrpSelProp = $$`<div class="flex mr-1 w-100">${$selProp}</div>`;
