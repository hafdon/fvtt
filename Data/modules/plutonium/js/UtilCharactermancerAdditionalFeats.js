...
class Charactermancer_AdditionalFeatsSelect extends BaseComponent {
	...
	render ($wrp) {
		const $wrpLeft = $(`<div class="flex-col w-100 h-100 min-h-0 overflow-y-auto"></div>`);
		const $wrpRight = $(`<div class="flex-col w-100 h-100 min-h-0 overflow-y-auto"></div>`);

		this.renderTwoColumn({$wrpLeft, $wrpRight});

		$$($wrp)`<div class="flex w-100 h-100 min-h-0">
			${$wrpLeft}
			<div class="vr-3"></div>
			${$wrpRight}
		</div>
	...
	renderTwoColumn ({$wrpLeft, $wrpRight}) {
		...
		const $stgGroupLeft = $$`<div class="flex-col"></div>`.appendTo($wrpLeft);
		const $stgGroupRight = $$`<div class="flex-col"></div>`.appendTo($wrpRight);
	...
	_render_$getStgSelGroup ({$wrpLeft}) {
		...
		return $$`<div class="w-100 mb-2 flex-v-center">
	...
	_render_renderPtStatic ({$stgGroupLeft, $stgGroupRight, featSet}) {
		...
		const rowMetas = uidsStatic.map((uid, ix) => {
			...
			const $stgFeatureOptions = this._isFeatureSelect ? $(`<div class="flex-col w-100"></div>`) : null;
			...
			const $rowRight = $(`<div class="flex-col w-100"></div>`);
	...
	_render_displayFeat ({$wrp, feat}) {
		$wrp.empty();
		if (!feat) {
			$wrp.append(`<div class="ve-muted mb-2 italic flex-vh-center">No feat selected.</div>`);
		} else {
	...
	_render_renderPtChooseFromFilter ({$stgGroupLeft, $stgGroupRight, featSet, lastMetas}) {
		...
		const rowMetas = [...new Array(featSet.any)].map((_, ix) => {
			...
			const $dispFeat = $(`<div class="flex-col w-100"></div>`);

			const $stgFeatureOptions = this._isFeatureSelect ? $(`<div class="flex-col w-100 mt-2"></div>`) : null;

			const $rowLeft = $$`<div class="flex-col w-100">
				<div class="bold mb-2">Select a Feat</div>
				<div class="flex-v-center btn-group w-100">${$btnFilterForFeat}${$selFeat}</div>
				${$stgFeatureOptions}
				<hr class="hr-1">
			</div>`;
	...
	async _pGetFormData_static ({out, featSet, ptrIsComplete}) {
		...
		for (let ix = 0; ix < uidsStatic.length; ++ix) {
			...
			if (!this._isFeatureSelect) continue;
