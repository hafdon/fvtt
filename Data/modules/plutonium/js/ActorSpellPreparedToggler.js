		...
		$html.find(`[name="btn-list-save"]`)
			.click(async () => {
				try {
					...
					await UtilDocuments.pUpdateDocument(this.actor, {flags: {[SharedConsts.MODULE_NAME_FAKE]: nxtFlags}});
					// endregion
		...
		$html.find(`[name="btn-list-load"]`)
			.click(async () => {
				try {
					...
					this._$wrpSaveRows = this._$wrpSaveRows || $(`<div class="flex-col w-100 h-100"></div>`);
...
ActorSpellPreparedToggler.RenderableCollectionSpellLists = class extends RenderableCollectionBase {
	...
	getNewRender (spellList) {
		...
		const $wrpRow = $$`<div class="flex-v-center py-1 stripe-even toggsp__row">
			<div class="mr-2">${$btnLoad}</div>
			<div class="flex-v-center w-100 mr-2">${comp._state.name}</div>
