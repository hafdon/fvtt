...
class SadnessChan {
    constructor() {
        this._minPlayerWhisperChance = 0;
        this._maxPlayerWhisperChance = 1;
        this._minDieType = 2;
        this._maxDieType = 1000;
        this._minCrtValue = 1;
        this._settingKeys = settingDefaults.SETTING_KEYS;
    }
    static getInstance() {
    ...
    _getStatsMessageBody(userData, statsBodyClass) {
        ...
        if (rolls) {
            ...
            const averageRollDie = () => {
                if (!Settings.getSetting(this._settingKeys.AVERAGE_TOGGLE))
                    return '';
    ...
    _shouldIWhisper(rolls, dieType, success, fail) {
        ...
        if (!(Math.random() < playerWhisperChance && rolls?.length))
            return false;
