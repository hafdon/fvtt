...
function checkRotationRateLimit(layer) {
    const hasTarget = layer.options?.controllableObjects
        ? layer.controlled.length
        : !!layer._hover
    if (!hasTarget) return false
    const t = Date.now()
    if (
        t - game.keyboard._wheelTime <
...
function zoom(event) {
    ...
    const dx =
        ((-t.tx + event.clientX) / canvas.stage.scale.x -
            canvas.stage.pivot.x) *
        (dz - 1)
    const dy =
        ((-t.ty + event.clientY) / canvas.stage.scale.y -
...
function panWithMultiplier(event) {
    const multiplier =
        (1 / canvas.stage.scale.x) * getSetting('pan-speed-multiplier')
    const invertVerticalScroll = getSetting('invert-vertical-scroll') ? -1 : 1
    const x = canvas.stage.pivot.x + event.deltaX * multiplier
    const y =
        canvas.stage.pivot.y + event.deltaY * multiplier * invertVerticalScroll
    canvas.pan({ x, y })
}
...
Hooks.on('init', function () {
    ...
    game.settings.register(MODULE_ID, 'zoom-around-cursor', {
        ...
        hint:
            'Center zooming around cursor. Does not apply to zooming with pageup or pagedown.',
        scope: 'client',
        config: true,
        default: true,
        type: Boolean,
...
||
    ...
    `,
        ...
        choices: {
            ...
            Alternative:
                'Alternative: can pan with Shift, rotate while holding Alt',
        },
        default: 'Default',
    })
    ...
    game.settings.register(MODULE_ID, 'invert-vertical-scroll', {
        ...
        hint:
            'If set to true, you will scroll up when dragging/scrolling down.',
        scope: 'client',
        config: true,
        default: false,
        type: Boolean,
    ...
    game.settings.register(MODULE_ID, 'min-max-zoom-override', {
        ...
        hint:
            'Override for the minimum and maximum zoom scale limits. 10 is the Foundry default (x10 and x0.1 scaling).',
        scope: 'client',
        config: true,
        default: 10,
        type: Number,
