...
class Tooltip {
    ...
    _appendObjectStat(values, item, stats) {
        if (isNaN(values.value) || isNaN(values.max) || values.value === null || values.max === null)
            return;
    ...
    _getActorDisplayName(staticData) {
        ...
        if (this._tooltipInfo.isGM && staticData.displayNameInTooltip)
            return tokenName;
        if (!this._tooltipInfo.isGM) {
            ...
            if (index === -1) {
                if (staticData.displayNameInTooltip === this._appKeys.NONE_DISPOSITION)
                    return null;
